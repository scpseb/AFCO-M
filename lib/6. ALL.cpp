/*
INTEGRASI SISTEM BATCH PROCESSING PENUH (ROTARY MANUAL + FILLING + CAPPING)
PlatformIO Version | ESP32-S3 | Dlloydev AnalogWrite (Servo)
*/

#include <Arduino.h>
#include <Servo.h> // Memanggil library Dlloydev yang sudah dipasang
...
const int PIN_SERVO_1 = 5;        // Channel 0 (Keran Utama)
...
void persiapanTangkiSementara() {
  ...
  // Buka keran tangki utama (Sintaks baru: write(pin, degree))
  servo1.write(PIN_SERVO_1, 45); 
  ...
  delay(1000); // Tunggu servo menutup sempurna
}
...
void eksekusiFilling() {
  ...
  if (digitalRead(PIN_IR_OUT_FILL) == LOW) { // Asumsi LOW = terdeteksi
    ...
    delay(4000); // Waktu pengisian 5 detik
    
    // Tutup keran pengisian botol
    servo2.write(PIN_SERVO_2, 90); 
    delay(1000); // Tunggu servo menutup rapat
...
void setup() {
  ...
  // Setup Pin Rotary & Stepper Capping
  ...
  // Inisialisasi Servo (Sintaks: attach(pin, channel))
  servo1.attach(PIN_SERVO_1, 0);
  servo2.attach(PIN_SERVO_2, 1);
  servoCapping.attach(PIN_SERVO_CAPPING, 2);
  
  // Posisi Awal Semua Servo Tertutup/Standby (Sintaks: write(pin, degree))
  servo1.write(PIN_SERVO_1, 90);
  servo2.write(PIN_SERVO_2, 90);
  servoCapping.write(PIN_SERVO_CAPPING, 0);
...
void loop() {
  Serial.println("\n=================================");
  Serial.println("MULAI SIKLUS BATCH BARU");
  Serial.println("=================================");
  ...
  Serial.println("Siklus selesai. Menunggu putaran berikutnya...");
  delay(2000); 
}